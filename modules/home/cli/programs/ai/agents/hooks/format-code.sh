...
find_project_root() {
    local dir="$1"
    # Look for package.json, .prettierrc*, or version control root
    while [[ "$dir" != "/" ]]; do
        if [[ -f "$dir/package.json" ]] || [[ -f "$dir/.prettierrc" ]] || [[ -f "$dir/.prettierrc.json" ]] || [[ -f "$dir/.prettierrc.js" ]] || [[ -d "$dir/.git" ]] || [[ -d "$dir/.jj" ]]; then
            echo "$dir"
            return 0
        fi
        dir=$(dirname "$dir")
