...
echo "Current work: $(jj log -r @ --no-graph -T 'bookmarks.join(", ") ++ " (" ++ change_id.short() ++ ")"')"
echo "Last change: $(jj log -r @ --no-graph -l 1 -T 'description.first_line()')"
echo "Status: $(jj status | head -5 | wc -l) files changed"
```
...
detect_and_add_context() {
    ...
    if jj status >/dev/null 2>&1; then
        context+="Current work: $(jj log -r @ --no-graph -T 'bookmarks.join(", ") ++ " (" ++ change_id.short() ++ ")"')\n"
        context+="Status: $(jj status | head -5 | wc -l) files changed\n"
        context+="Available commands: jj help, jj worklog, jj status\n"
    fi
...
check_security() {
    local operation="$1"
    local current_bookmarks=$(jj log -r @ --no-graph -T 'bookmarks.join(", ")' 2>/dev/null)
    local is_production=false

    # Detect production environment
    if [[ "$MIX_ENV" == "prod" ]] || \
       [[ "$NODE_ENV" == "production" ]] || \
       [[ "$current_bookmarks" == *"main"* ]] || \
       [[ "$current_bookmarks" == *"master"* ]] || \
       [[ "$current_bookmarks" == *"prod"* ]]; then
        is_production=true
    fi
...
{
  "continue": false,
  "stopReason": "⛔ BLOCKED: Dangerous database operation detected in production environment. Use 'jj new' or 'jj edit <feature-bookmark>' to switch to development work."
}
EOF
            exit 2
        fi
